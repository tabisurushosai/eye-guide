...
- `src/core/**/*.ts` is guarded by lint against `chrome` global usage and by `tsconfig.core.json`, which runs without Chrome ambient types during `npm run build`. Keep any future platform SDK access in entrypoints or adapters.
...
Use `STORAGE_KEYS` from `src/storage/types.ts` when an entrypoint or adapter needs a storage key. This keeps key spelling in one platform-neutral place and preserves the existing serialized names.
